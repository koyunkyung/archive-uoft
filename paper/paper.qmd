---
title: "A Poll-of-Polls Forecast for the 2024 U.S. Presidential Election: Analysis of Support Trends for Harris Accounting for Pollster and State Disparities"
subtitle: "Kamla Harris Emerges as a Leading Candidate with Constant Support Rates Hovering Around 50.03%"
author: 
  - Yunkyung Ko
...
date: today
date-format: long
abstract: "In the anticipation of the 2024 US presidential election, this paper intends to provide a trend analysis of the support rates for Harris accounting for pollster and state specific factors. Supportive polls for Harris remained relatively constant around 50.03%. However, some polling organizations and battleground states such as Siena/NYT and Michigan, showed variations from the average national results. Our objective is to propose a reliable electoral prediction ny not only analyzing the summary statistics and correlations of date, pollster, and state variables, but also by constructing prediction models using a linear and a Bayesian model approach."
format: pdf
...
#| fig-cap: "Polling concentration over time by pollster *Note:* Concentrated polling efforts of Siena/NYT in the first half of September, 2024 show temporal bias and potential overemphasizing of their polling methodologies around key election events."
#| echo: false
...
plot_usmap(data = pollster_state_data, values = "total_polls", regions = "states") +
  facet_wrap(~ pollster, ncol = 2) +  # Create a map for each pollster
...
## Results from the relation between the prediction variable and outcome variable of the analysis data
...
Prediction results derived from our model frameworks are summarized in @tbl-modelresults1 and @tbl-modelresults2. In the linear models (@tbl-modelresults1)
